...
// Copied from
// https://source.chromium.org/chromium/chromium/src/+/main:net/quic/dedicated_web_transport_http3_client.cc
// with modifications.
...
class ConnectStream : public ::quic::QuicSpdyClientStream {
 public:
  ConnectStream(::quic::QuicStreamId id,
                ...
                WebTransportHttp3Client* client)
 ...
 private:
  WebTransportHttp3Client* client_;
};

class WebTransportHttp3ClientSession : public ::quic::QuicSpdyClientSession {
 public:
  WebTransportHttp3ClientSession(
...
WebTransportHttp3Client::WebTransportHttp3Client(
    const GURL& url,
...
::quic::QuicSpdyClientSession* WebTransportHttp3Client::quic_session() {
  return session_.get();
}

void WebTransportHttp3Client::DoLoop(int rv) {
  do {
...
int WebTransportHttp3Client::DoCheckProxy() {
  ...
  return context_->proxy_resolution_service()->ResolveProxy(
      ...
      base::BindOnce(&WebTransportHttp3Client::DoLoop, base::Unretained(this)),
      &proxy_resolution_request_, net_log_);
}

int WebTransportHttp3Client::DoCheckProxyComplete(int rv) {
  if (rv != OK)
    return rv;
...
int WebTransportHttp3Client::DoResolveHost() {
...
int WebTransportHttp3Client::DoResolveHostComplete(int rv) {
  if (rv != OK)
    return rv;
...
int WebTransportHttp3Client::DoConnect() {
  int rv = OK;
...
void WebTransportHttp3Client::CreateConnection() {
  ...
  session_ = std::make_unique<WebTransportHttp3ClientSession>(
      InitializeQuicConfig(*quic_context_->params()), supported_versions_,
      connection.release(),
      ::quic::QuicServerId(url_.host(), url_.EffectiveIntPort()),
      &crypto_config_, &push_promise_index_, this);
...
int WebTransportHttp3Client::DoConnectComplete() {
...
void WebTransportHttp3Client::OnSettingsReceived() {
  DCHECK_EQ(next_connect_state_, CONNECT_STATE_CONNECT_COMPLETE);
  // Wait until the SETTINGS parser is finished, and then send the request.
  task_runner_->PostTask(FROM_HERE,
                         base::BindOnce(&WebTransportHttp3Client::DoLoop,
                                        weak_factory_.GetWeakPtr(), OK));
}

void WebTransportHttp3Client::OnHeadersComplete() {
...
int WebTransportHttp3Client::DoSendRequest() {
  ...
  WebTransportHttp3ClientSession* session =
      static_cast<WebTransportHttp3ClientSession*>(session_.get());
...
int WebTransportHttp3Client::DoConfirmConnection() {
  if (!session_ready_) {
    return ERR_METHOD_NOT_SUPPORTED;
  }
...
void WebTransportHttp3Client::TransitionToState(WebTransportState next_state) {
...
void WebTransportHttp3Client::OnIncomingUnidirectionalStreamAvailable() {
  visitor_->OnIncomingUnidirectionalStreamAvailable();
}

void WebTransportHttp3Client::OnDatagramReceived(absl::string_view datagram) {
  visitor_->OnDatagramReceived(base::StringViewToStringPiece(datagram));
}

void WebTransportHttp3Client::OnCanCreateNewOutgoingBidirectionalStream() {
  visitor_->OnCanCreateNewOutgoingBidirectionalStream();
}

void WebTransportHttp3Client::OnCanCreateNewOutgoingUnidirectionalStream() {
  visitor_->OnCanCreateNewOutgoingUnidirectionalStream();
}
...
bool WebTransportHttp3Client::OnPacket(
...
int WebTransportHttp3Client::HandleWriteError(
    int error_code,
...
void WebTransportHttp3Client::OnWriteUnblocked() {
  connection_->OnCanWrite();
}

void WebTransportHttp3Client::OnConnectionClosed(
  ...
  if (!retried_with_new_version_ &&
    ...
    if (!supported_versions_.empty()) {
      ...
      // instead.
      task_runner_->PostTask(
          FROM_HERE, base::BindOnce(&WebTransportHttp3Client::CreateConnection,
                                    weak_factory_.GetWeakPtr()));
      return;
    }
